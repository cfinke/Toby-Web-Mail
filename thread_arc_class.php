	...
	function export_image(){
		if ($this->thread->num_messages > 1){
			...
			foreach($this->thread->flat_thread as $key => $value){
				...
				if (is_array($replies)){
					foreach($replies as $reply){
						...
						$arc_w = round($this->circle_width * 1.7) * $depth;
						$arc_h = $arc_w;
						
						// Set the horizontal center of the arc.
						if ($this->orientation == "h"){
							$center_x = round($this->circle_width  * 3 / 4) + ($center_at_circle * ($this->circle_width + 10));
						}
						else{
						...
						if (($generation % 2) == 0){
							...
							if ($this->orientation == "h"){
								$center_y = round(($this->image_height - $this->circle_width) / 2);
							}
							else{
							...
							// Draw the arc.
							if ($this->orientation == "h"){
								imagearc($this->image, $center_x, $center_y, $arc_w, $arc_h, 180, 0, $color);
							}
							else{
							...
							// Draw the arc.
							if ($this->orientation == "h"){
								imagearc($this->image, $center_x, $center_y, $arc_w, $arc_h, 0, 180, $color);
							}
							else{
			...
			for ($i = 0; $i < $this->thread->num_messages; $i++){
				...
				if ($this->orientation == "h"){
					imagefilledellipse($this->image, round($this->circle_width  * 3 / 4) + ($i * ($this->circle_width + 10)), ($this->image_height / 2), $this->circle_width + $extra_w, $this->circle_width + $extra_w, $color);
				}
				else{
				...
				if ($this->orientation == "h"){
					if (in_array($this_key, $this->thread->sent)) imagefilledellipse($this->image, round($this->circle_width  * 3 / 4) + ($i * ($this->circle_width + 10)), ($this->image_height / 2), $this->circle_width - (2 * $this->circle_border_width) + $extra_i, $this->circle_width - (2 * $this->circle_border_width) + $extra_i, $white);
				}
				else{
			...
			if ($this->orientation == "h"){
				...
				$image_y = round($this->image_height / 2) - round($this->circle_width * 0.5) - ($top_depth  * $this->circle_width) - 5;
				
				// Determine the highest point at which there is data on the image.
				$image_y_bottom = round($this->image_height / 2) + round($this->circle_width * 0.5) + ($bottom_depth * $this->circle_width) + 5;
				
				// Get the height of the new image.
				$new_image_h = $image_y_bottom - $image_y;
				
				// Create a new image to crop the current image.
				$new_image = imagecreate($this->image_width, $new_image_h);
				
				// Copy the pertinent section of the old image to the new image.
				imagecopy($new_image, $this->image, 0, 0, 0, $image_y, $this->image_width, $new_image_h);
			}
			else{
				$depth = ($bottom_depth > $top_depth) ? $bottom_depth : $top_depth;
				
				// Determine the narrowest point at which there is data on the image.
				$image_x = round($this->image_width / 2) - round($this->circle_width * 0.5) - ($depth  * $this->circle_width) - 5;
				...
				// Get the height of the new image.
				$new_image_width = $image_x_bottom - $image_x;
		...
		if ($this->thread->num_messages > 1){
			...
			for ($i = 0; $i < $this->thread->num_messages; $i++){
				...
				if ($this_key != $this->selected_key){
					...
					if ($this->orientation == "h"){
						// Horizontal map
						$this->map .= '<area shape="rect" coords="'.round($this->circle_width * 1.6 * $i).',0,'.round($this->circle_width * 1.6 * ($i + 1)).','.$this->image_height.'" href="'.$_SERVER["PHP_SELF"].'?action=view&amp;id='.$this_id.'" alt="'.$this->get_subject($this_key, $this->thread->thread).'" />'."\n";
					}
					else{
						// Vertical map
						$this->map .= '<area shape="rect" coords="0,'.round($this->circle_width * 1.6 * $i).','.$this->image_height.','.round($this->circle_width * 1.6 * ($i + 1)).'" href="'.$_SERVER["PHP_SELF"].'?action=view&amp;id='.$this_id.'" alt="'.$this->get_subject($this_key, $this->thread->thread).'" />'."\n";
					}
				}
			}
